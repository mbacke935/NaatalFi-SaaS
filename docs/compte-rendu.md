...
**Date :** 11 juin 2026  
**État :** Phases 0 à 11 implémentées, déploiement Render/Vercel engagé.
...
| `shipping` | Terminé Phase 11 | `ShippingZone`, `ShippingRate`, CRUD vendeur, estimation livraison, intégration checkout |
...
| Dashboard vendeur | Boutique, produits, commandes reçues, changement statut, portefeuille (soldes + transactions + retraits), livraison (zones + tarifs) |
...
1. Déployer Phases 10-11 (wallet + shipping) sur Render + Vercel.
2. Phase 12 : Dashboard vendeur complet (sidebar nav, KPIs /dashboard, profil, notifications).
3. Phase 13 : Dashboard admin complet (wallets, retraits, analytics globales).
